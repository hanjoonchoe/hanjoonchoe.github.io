...
>Base case n=1<br>
>trivially true<br>
>Assume that $n=k$ is true,that is, there exist $\widetilde{A} \in \mathcal{M}_{k \times k} such that $\widetilde{A} = \widetilde{U}\widetilde{T}\widetilde{U}$ , where $\widetilde{U}$ is unitary matrix and $\widetilde{T}$ is $k \times k$ upper trinagular matrix.<br>
