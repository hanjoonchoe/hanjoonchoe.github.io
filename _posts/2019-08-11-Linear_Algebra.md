...
#### 만약 V가 finite-dimensional이고 $U$가 $V$의 subspace라면 $V = U \oplus W$가 되는 또다른 subspace $W$가 존재한다.
  ...
  >$(a_{1}u_{1}+...a_{j}u_{j})-(b_{1}u_{1}+b_{k}u_{k})=0$ implies $a_{1}=...=a_{j}=b_{1}=...=b_{k}=0$<br>
...
>Base case n=1<br>
>trivially true<br>
>Assume that $n=k$ is true,that is, there exists $\widetilde{A} \in \mathcal{M}_ {k}$ such that $\widetilde{A} = \widetilde{U}\widetilde{T}\widetilde{U}^{\ast} \iff \widetilde{U}^{\ast}\widetilde{A}\widetilde{U} = \widetilde{T} $ , where $\widetilde{U}$ is $k \times k$ unitary matrix ,and $\widetilde{T}$ is $k \times k$ upper trinagular matrix.<br>
