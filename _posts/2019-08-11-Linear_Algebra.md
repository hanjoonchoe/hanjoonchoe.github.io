...
>Choose normalized eigenvector $v_{1}$,which is $\parallel v_{1} \parallel_{2} = 1$, corresponding to eigenvalue $\lambda_{1}$. We can construct an orthonormal basis of A(we can use gram-schmidt process), then $ U = \[v_{1} \ v_{2} \ v_{3} \ \cdots v_{n} \]$ forms unitary matrix.<br>
>Set $T = $ $U^{\ast}$ $\begin{array}{c|c} \lambda_{1} & \ast   \ \ \   \cdots   \ \ \   \ast \\\ \hline 0 &  \\\ \vdots &   \ \ \widetilde{A} \ \\\ 0 & \end{array}$ $U$, then $\begin{array}{c|c} Uv_{1}U^{\ast} &  \\\ \hline 0 &  \\\ \vdots & \end{array}$
