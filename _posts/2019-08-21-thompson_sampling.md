...
t trial에서 각 choice에 대한 $\hat{\theta_{k}}$값들(다르게 말해 성공확률들)이 구해질 것이고, 이 확률들 중 가장 확률을 가지는 $\hat{\theta_{k}}$를 선택한다.<br>
가장 큰 성공확률을 나타내는 $\hat{\theta_{k}}$가 선택 되었으면 t trial에서 $\hat{\theta_{k}}$에 대응되는 choice를 $x_{t}$로 두고 이에 대한 reward값인 $r_{t}$ 확인한다.<br>
...
즉, $\hat{\theta_{k}}$의 sampling distribution의 variance가 작아진다. 따라서 각 분포에 대한 $\hat{\theta_{k}}$값이 균질해진다.<br> 그리고, 이러한 효과는 낮은 성공확률과 높은 성공확률을 가지는 분포들에서 뽑은 random sample $\hat{\theta_{k}}$를 더욱 명백히 하는 효과를 준다.
