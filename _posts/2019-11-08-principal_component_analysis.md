...
Diagonalizable이므로 eigenvalue decomposition이 가능하고 이 말은 $P^{-1}AP$ 꼴로 표현이 가능하다는 것을 말한다. 여기서 $P$는 orthogonal basis로 구성된 Matrix이다. Orthogonal Matrix의 특성은 $P^{-1}P = I = P^\topP$ 그리고 $A$는 Diagonal Matrix이며 각각의 entry는 P의 orthogonal basis의 scaling을 담당하는 역할을 한다. 다르게 말해서, $P$의 orthogonal basis들은 eigenvector 역할을 $A$의 diagonal entry들은 eigenvalue역할을 한다.
