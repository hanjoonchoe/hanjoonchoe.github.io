...
$\bm{x}_i = {x_1,...,x_n}$ 그리고 $\bm{x}_i \in X$

따라서 , 분산은 $n \times n$의 Matrix로 표현되고 정방대칭 행렬이다. 왜냐하면 $(\bm{x}_i-\mu)(\bm{x}_j-\mu)^\top = (\bm{x}_j-\mu)(\bm{x}_i-\mu)^\top$ for all $i,j$
...
$\max_{\lambda_i} (\bm{a}_i\bm{x}_i)^{\top}D(\bm{a}_i\bm{x}_i}$
