...
$\max_{\lambda_i} \textbf{x}_i)^{\top}\Sigma(\textbf{x}_i}$ 여기서 $\textbf{x}_i$는 $\Sigma$를 eigenvalue decomposition했을 때의 orthonormal basis이고 \lambda_i는 i번째 orthogonal basis의 eigenvalue이다.<br>
그렇다면 $\max_{\lambda_i} \textbf{x}_i) \Sigma (\textbf{x}_i}^{\top}$ = $\textbf{x}_i) \textbf{x}_i^{\top} D \textbf{x}_i (\textbf{x}_i}^\top$
<br>
