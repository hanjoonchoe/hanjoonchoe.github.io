...
$\Sigma$는 음이 아닌 정수<- 잘못 적었다 (아이겐벨류 값이 음수가 아니다...) entry를 가진 정방대칭행렬이다.<- 아마 EVD했을때 대각행렬이 음수가 아니라는 것을 말하고 싶었던 것 같다... 그말인 즉, positive semi definite(PSD)이다. <- 좀 더  정의는 $a\Sigma a^\top \succeq 0$<br>
