...
방법은 단순한 편이다.<br>
Choice set $\mathcal{C} = \lbrace c_{1},c_{2},...,c_{k} \rbrace$가 있다고 한다면 각 $theta_{i}$(feature)와 $c_{j}$(choice)에 대해 특정한 $\mu_{i}$ 와 $\sigma_{i}^2$를 가지는 정규분포를 따른다고 가정하고 각각의 확률을 정규분포 pdf(probability density function)로 치환해 주면된다.
