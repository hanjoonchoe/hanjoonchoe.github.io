...
따라서 $y_{i}(\vec{w}^{T}\vec{x}_ {i}+b) \geq 1$라는 조건은 data point들이 아랫쪽 hyperplane에 맞닿거나 그 아래 그렇지 않으면 위쪽 hyperplane에 맞닿거나 그 위에 위치해야 한다는 것을 의미한다. 만약 아래쪽 hyperplane의 위쪽 그리고 위쪽 hyperplane의 아래쪽에 data point가 자리하고 있으면 부등식이 성립하지 않는다.
