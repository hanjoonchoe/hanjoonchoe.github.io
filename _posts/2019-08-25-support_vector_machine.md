...
$ \iff w - \sum_{i=1}^{n} \lambda_{i} y_{i} \vec{x}_ {i}  = 0$<br><br>
...
위의 두 결과
1. $ w = \sum_{i=1}^{n} \lambda_{i} y_{i} \vec{x}_ {i}$<br>

을 $g(\lambda)$에 대입해 주면

$g(\lambda) = \inf(\frac{1}{2} (\sum_{i=1}^{n} \lambda_{i} y_{i} \vec{x}_ {i})^{T} (\sum_{i=1}^{n} \lambda_{i} y_{i} \vec{x}_ {i}) - \sum_{i=1}^{n}\sum_{j=1}^n}(\lambda_{i} \lambda_{i}\lambda_{j}y_{i}y_{j}{\vec{x}_ {i}}^{T}{\vec{x}_ {j}}+\lambda_{i}y_{i}b-\lambda_{i}))$
