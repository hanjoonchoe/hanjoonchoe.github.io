...
그리고 이것은 $g(\lambda, \upsilon)$가 global optimal(infimum) point $x^{\ast}$가 존재한다는 말과 동치이다.
...
$\qquad\qquad = \inf\limits_{x \in \mathcal{D}}((c^{T}+\lambda^{T}A+\upsilon^{T})x - \lambda^{T}b)$
